...
from . import logger
from .models import Entities, EntitiesSearch
...
class SearchManage(EntitiesSearch):
    ...
    def __init__(self, url_api: str = URL_WIKI, key: Literal['search', 'ids'] = 'search', m_num: int = 10):
    ...
    def __set_get_entities(self, points: list, **kwargs):
        ...
        """
        m_num = min(self.m_num, len(points))
        self.entities_num = len(points)
    ...
    def set_entities_params(self, **kwargs):
        ...
        if self.search_queue.empty():
            logger.warning("Search queue is empty. Can not change parameters of entities.")
            return
        queue_t = Queue[Entities]()
        while not self.search_queue.empty():
    ...
    def analysis_entities(self, keys: Union[str, List[str], None] = None) -> dict:
    ...
    def search_run(self, points: list, keys: Union[str, List[str], None] = None,
        ...
        log_type ='text' if self.key == 'search' else "wiki's entities id"
        logger.info(f"Entities: {self.entities_num} (type: {log_type}). Threading number: {self.m_num}.")
...
class SparqlQuery(EntitiesSearch):
    ...
    def __init__(self, m_num: int = 10, format_: str = 'json', url_: str = URL_SPARQL_WIKI, sparql_: str = SPARQL_WIKI):
        super().__init__(key='sparql', m_num=m_num, paramFormat=sparql_, name='sparql')
    ...
    def analysis_to_dict(self) -> dict:
        re_an = {key: [value] for key, value in self.re_list[0].get_analysis.items()}
        da_: Entities
        for da_ in self.re_list[1::]:
            for key, value in da_.get_analysis.items():
                re_an[key].append(value)
...
class BingQuery(EntitiesSearch):
    ...
    def __init__(self, url_: str = URL_BING, m_num: int = 10):
        super().__init__(key="q", m_num=m_num, paramFormat=PARAM_BING_QUERY, name='bingQuery')
        self.__url_ = url_
    ...
    def search_run(self, points: list, timeout: float = 30.0, time_stop: float = 30.0,
        ...
        self.init_queue(points, **kwargs)
        function_ = function_ or AnalysisTools.bing_search_analysis
...
class SpellCheck(EntitiesSearch):
    ...
    def __init__(self, url_: str = URL_BING, m_num: int = 10):
        super().__init__(key="q", m_num=m_num, paramFormat=PARAM_BING_SPELL_CHECK, name=f'SpellCheck<{url_}>')
        self.__url_ = url_
    ...
    def search_run(self, points: list, timeout: float = 30.0, time_stop: float = 30.0,
        ...
        function_ = function_ or AnalysisTools.spell_check_bing
        try:
            self.multithread_get_(timeout=timeout, time_stop=time_stop, block_num=block_num,
                                  url=self.__url_, keys=None, function_=function_, args=args)
        except RuntimeError:
...
class DbpediaLookUp(EntitiesSearch):
    ...
    def analysis_to_dict(self) -> dict:
        ...
        :return: data of dict[list] from list[dict]
        """
        re_an = {k: [v] for k, v in self.re_list[0].get_analysis.items()}
        for da_ in self.re_list[1::]:
            for key, value in da_.get_analysis.items():
                re_an[key].append(value)
        return re_an
    ...
    def search_run(self, points: list, patten: str = "search", is_all: bool = False, timeout: float = 30.0,
        ...
        """
        self.init_queue(points, **kwargs)
        url_ = URL_DBPEDIA_SEARCH if patten == 'search' else URL_DBPEDIA_PREFIX
        if function_ is None:
        ...
        dict_t = {
            k_: Tools.list_back(v_, self.index_)
            for k_, v_ in self.analysis_to_dict().items()
        }
        if is_all:
            return dict_t
