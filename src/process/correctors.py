...
from ..searchmanage.SearchManage import SpellCheck
from ..searchmanage.tools import AnalysisTools
from ..utils import config, logger
from ..utils.lists import remove_false, unique
...
class SpellChecker:
    ...
    @classmethod
    def spell_check_process(cls, texts: Iterable[str], checkers: list[CheckRequester], *, force: bool = False) -> set[str]:
    ...
    def repeated_search(cls, to_correct: set[str], checkers: list[CheckRequester]) -> None:
        ...
        def chunked(values: list[tuple[str, str]], chunk_size: int):
            for j in range(0, len(values), chunk_size):
                logger.info(
                    "  - Spell check request chunk %d/%d of size %d.", j // chunk_size + 1,
                    (len(values) + chunk_size - 1) // chunk_size, chunk_size
                )
    ...
    @classmethod
    def _chunked_search(cls, to_correct: list[tuple[str, str]],
                        checkers: list[CheckRequester]) -> list[tuple[str, str]]:
    ...
    def _process_result(cls, text_orig: str, texts_pre: str, re_check: list[str]) -> None:
        ...
        choices = [s for s in {*res_, *prod, text_orig.lower(), texts_pre.lower()} if len(s) >= len(text_orig) / 2]

        final_res = [t[0] for t in process.extract(text_orig.lower(), choices, limit=cls.correction_limit)]
